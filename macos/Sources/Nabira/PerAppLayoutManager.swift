...
final class PerAppLayoutManager {
    ...
    func start() {
        ...
        guard observer == nil else { nabiraLog("PerAppLayout: already started"); return }
        ...
        nabiraLog("PerAppLayout: started")
    }
    ...
    func stop() {
        ...
        nabiraLog("PerAppLayout: stopped")
    }
    ...
    private func handleAppActivated(_ app: NSRunningApplication?) {
        ...
        if let savedLayout = layoutByApp[newBundleID], savedLayout != currentLayout {
            nabiraLog("PerAppLayout: \(newBundleID) → restore \(savedLayout)")
