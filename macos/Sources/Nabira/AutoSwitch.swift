...
enum LayoutVerdict: Equatable { case switchToConverted, keep, undecided }
...
enum AutoSwitchPolicy {
    ...
    static func isDeniedApp(_ bundleID: String?) -> Bool {
        ...
        return matchesDeniedApp(id, entries: SettingsManager.shared.deniedApps)
    }

    static func matchesDeniedApp(_ bundleID: String, entries: [String]) -> Bool {
        for entry in entries {
            if entry.hasSuffix("*") {
                if bundleID.hasPrefix(String(entry.dropLast())) { return true }
    ...
    /// на ДРУГОЙ машине — наш инстанс должен молчать и уступить удалённому Nabira.
