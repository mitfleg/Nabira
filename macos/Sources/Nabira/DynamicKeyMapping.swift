...
enum DynamicKeyMapping {
    ...
    static func convert(_ inputText: String) -> String {
        ...
        if text.unicodeScalars.contains(where: { $0.properties.generalCategory == .nonspacingMark }) {
            nabiraLog("DynamicKeyMapping: combining marks in text — bail")
            return inputText
        }
        let settings = SettingsManager.shared
        let layouts = LayoutSwitcher.installedLayouts()
        ...
        guard let source = layouts.first(where: { LayoutSwitcher.sourceID($0) == currentID }),
            ...
            guard pairIsStaticSafe(layouts: layouts, id1: layout1ID, id2: layout2ID) else {
                nabiraLog("DynamicKeyMapping: pair unresolved and not en/ru — no static fallback")
                return inputText
            }
            nabiraLog("DynamicKeyMapping: fallback to static mapping")
        ...
        if map.isEmpty {
            guard pairIsStaticSafe(layouts: layouts, id1: layout1ID, id2: layout2ID) else {
                nabiraLog("DynamicKeyMapping: empty map, pair not en/ru — no static fallback")
                return inputText
            }
            nabiraLog("DynamicKeyMapping: empty map, fallback to static")
            return KeyMapping.convert(text)
        }
