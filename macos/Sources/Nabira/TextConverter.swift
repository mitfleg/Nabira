...
final class TextConverter {
    ...
    nonisolated private func makeSource() -> CGEventSource? {
        ...
        source?.userData = kNabiraEventMarker
        return source
    }
    ...
    private func isFocusedElementEditable() -> Bool {
        ...
        if textRoles.contains(role) {
            ...
            if editErr == .success, let editable = editableRaw as? Bool {
                nabiraLog("editable: role=\(role) editable=\(editable)")
                return editable
            }
            nabiraLog("editable: role=\(role) (no AXEditable attr, assuming yes)")
            return true
        }

        nabiraLog("editable: role=\(role) — not a text field")
        return false
    }
    ...
    /// приложению — его штатный хоткей не теряется.
    func handlePlainTextPaste(allowPlainText: Bool = true) {
        guard allowPlainText, !isConverting, isFocusedElementEditable() else {
            simKey(keyCode: KC.letterV, flags: [.maskCommand, .maskShift])
            return
        }

        // Если другая clipboard-операция ещё ждёт восстановления, сначала возвращаем
        // реальный пользовательский буфер, иначе сняли бы наш временный текст.
        restoreClipboardNow()
        ...
        savedClipboardItems = originalItems
        isConverting = true
        clearState()
        ...
        // Большинство приложений читает pasteboard синхронно с Cmd+V. Небольшой запас
        // покрывает Electron/WebView; changeCount не даёт затереть новый Copy пользователя.
        let restoreWork = DispatchWorkItem { [weak self] in
            ...
            if pasteboard.changeCount == temporaryChangeCount {
                ...
                nabiraLog("plain paste: clipboard restored (\(originalItems.count) items)")
            } else {
                nabiraLog("plain paste: clipboard changed by user/app — keeping new contents")
            }
            self.savedClipboardItems = nil
            self.clipboardRestoreWork = nil
            self.isConverting = false
        }
        clipboardRestoreWork = restoreWork
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.25, execute: restoreWork)
    }
    ...
    func convert(wordKeys: [TypedKey], prevWordKeys: [TypedKey], boundaryCount: Int,
        ...
        guard let pair = DynamicKeyMapping.convertKeys(keys) else {
            ...
            guard passthroughSuffix.isEmpty else {
                nabiraLog("buffer convert: suffix + unresolved layouts — bail")
                return false
            }
            nabiraLog("buffer convert: layouts not resolved — fallback to clipboard")
        ...
        lastWasBuffer = true
        reconvertShouldSwitchLayout = true
        nabiraLog("buffer convert: \(keys.count) keys (+\(passthroughSuffix.count) punct, +\(trailingSpaces) sp)")
    ...
    /// Заменяет завершённое слово по уже вычисленному результату. Используется ёфикатором
    /// и сохраняет тот же быстрый, clipboard-free движок, что конвертация раскладки.
    func replaceCompletedWord(original: String, replacement: String, boundaryCount: Int,
                              passthroughSuffix: String = "",
                              changesLayout: Bool = false) -> Bool {
        guard !isConverting, !original.isEmpty, original != replacement, boundaryCount >= 0 else { return false }
        isConverting = true

        let spaces = String(repeating: " ", count: boundaryCount)
        let insert = replacement + passthroughSuffix + spaces
        lastOriginal = original + passthroughSuffix + spaces
    ...
    /// Доставляет Enter после всех уже запланированных замен. Та же serial queue гарантирует
    /// строгий порядок: Backspace → исправленный текст → Enter, без таймеров и гонок.
    func forwardKeyAfterPendingOperations(keyCode: UInt16, flags: CGEventFlags) {
        injectQueue.async { [weak self] in
            self?.simKey(keyCode: keyCode, flags: flags)
        }
    }
    ...
    func convertLine() -> Bool {
        ...
        guard isFocusedElementEditable() else { nabiraLog("convertLine: non-editable focus — bail"); return false }
    ...
    func convertLineBuffer(_ lineKeys: [TypedKey]) -> Bool {
        ...
        nabiraLog("line convert (terminal buffer): \(original.count)→\(converted.count) chars")
        injectQueue.async { [weak self] in
            guard let self else { return }
            self.backspace(bsCount)
            usleep(8_000)   // короткий зазор стирание→вставка: порядок и так гарантирован очередью HID
    ...
    private func cycleCaseByBuffer(_ original: String) -> Bool {
        ...
        let textToInsert = changed                            // immutable capture для Swift 6 Sendable
        isConverting = true
        nabiraLog("case: buffer \(original.count) chars → variant \(idx), bs \(bs)")
        injectQueue.async { [weak self] in
            guard let self else { return }
            self.backspace(bs)
            usleep(20_000)
    ...
    func changeCaseLine() -> Bool {
        guard !isConverting else { return false }
        guard isFocusedElementEditable() else { nabiraLog("changeCaseLine: non-editable — bail"); return false }
    ...
    func convertSpotlight() -> Bool {
        ...
        guard let text = tryCopy(pasteboard), !text.isEmpty else {
            nabiraLog("spotlight convert: read failed")
        ...
        guard converted != text else {
            nabiraLog("spotlight convert: no-op — bail")
        ...
        nabiraLog("spotlight convert: \(text.count)→\(converted.count) chars (Cmd+A + clipboard)")
    ...
    func convertSpotlightWord(converted: String, boundaryCount: Int) -> Bool {
        ...
        reconvertShouldSwitchLayout = true
        nabiraLog("spotlight auto: word-select replace (bc=\(boundaryCount))")
        injectQueue.async { [weak self] in
            guard let self else { return }
    ...
    func convertViaClipboard(wordLength: Int, prevWordLength: Int, boundaryCount: Int,
        ...
        if !isFocusedElementEditable() {
            nabiraLog("convert: element may not be editable, trying anyway")
        }
        let pasteboard = NSPasteboard.general
        cancelClipboardRestore()
        ...
        if let text = tryCopy(pasteboard, attempts: selectionCopyAttempts) {
            ...
            if converted == text {
                nabiraLog("convert: no-op conversion — bail")
                return false
            }
            pasteText(converted, pasteboard: pasteboard)
        ...
        if let l = LayoutSwitcher.currentAndOppositeLanguage(),
           LayoutDetector.isHebrew(l.current) || LayoutDetector.isHebrew(l.opposite) {
            nabiraLog("convert: hebrew pair — count-based clipboard path unsafe, bail")
            return false
        }
        ...
        if wordLength > 0 {
            ...
            nabiraLog("convert: nothing to convert (wordLen=\(wordLength) prevLen=\(prevWordLength))")
            return false
        }

        nabiraLog("convert: selecting \(charCount) chars (boundary=\(usedBoundary))")
        selectBack(charCount)
        usleep(50_000)

        guard let text = tryCopy(pasteboard) else {
            nabiraLog("convert: copy failed")
            simKey(keyCode: KC.right, flags: []) // снять выделение
            moveRight(usedBoundary)
            return false
        }

        nabiraLog("convert: word len=\(text.count)")
    ...
    private func reconvertViaClipboard() -> Bool {
        guard !isConverting else {
            nabiraLog("reconvert: skipped — already converting")
            return false
        }
        isConverting = true
        defer { isConverting = false }

        nabiraLog("reconvert: lastCount=\(lastConvertedCount) boundary=\(lastBoundaryCount)")
        ...
        guard !lastClipboardRTL else {
            nabiraLog("reconvert: RTL selection — arrow-based path unsafe, bail")
            return false
        }
        ...
        guard let text = tryCopy(pasteboard) else {
            nabiraLog("reconvert: copy failed, count=\(lastConvertedCount)")
            simKey(keyCode: KC.right, flags: [])
        ...
        if text == lastConverted, !lastOriginal.isEmpty {
            ...
            nabiraLog("reconvert: restored original (len=\(converted.count))")
        } else {
            converted = TextConverter.normalizedForInsert(DynamicKeyMapping.convert(text))
            nabiraLog("reconvert: len=\(text.count) → converting (fallback)")
        }
        pasteText(converted, pasteboard: pasteboard)
    ...
    private func scheduleClipboardRestore() {
        ...
        let work = DispatchWorkItem { [weak self] in
            ...
            self?.savedClipboardItems = nil
            nabiraLog("clipboard restored (\(saved?.count ?? 0) items)")
        }
