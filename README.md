...
Точность важнее полноты: однобуквенное слово определяется только по следующему надёжному слову (`F ns` → `А ты`, но `I From` остаётся английским), а частые двухбуквенные слова — по компактному белому списку. Неизвестные сокращения капсом приложение пропускает, но проверенный список технических терминов обрабатывается без системного словаря: `мзт` превращается в `VPN`, а `vpn` получает правильный регистр `VPN`. Слова с цифрами / URL / пунктуацией в середине, camelCase / смешанные алфавиты (идентификаторы кода), терминалы / IDE / менеджеры паролей и поля паролей пропускаются. Для RU/EN используются встроенные частотные словари, быстрый индекс вариантов с одной опечаткой и компактная контекстная ONNX-модель. Другие языки по-прежнему опираются на системный словарь macOS; если его нет, остаётся ручной триггер.
...
Встроенный корректор после пробела исправляет уверенные ошибки: `превет` → `привет`, `Карова` → `Корова`, `bokk` → `book`, `adress` → `address`, `iphnoe` → `iphone`. Встроенный индекс Symmetric Delete быстро находит варианты с пропуском, лишней буквой, заменой или соседней перестановкой; системный словарь добавляет свои варианты, если доступен. Локальные частотные словари RU/EN и анализ структуры ошибки выбирают безопасный результат. Функция работает офлайн, включена по умолчанию, сохраняет регистр, учитывает исключения и отменяется триггером конвертации. Правильные слова, имена, сокращения капсом, camelCase и смешанные идентификаторы не меняются.

Редкие, составные и сленговые слова могут отсутствовать и во встроенных, и в системных словарях. Ровно для этого нужны списки исключений: часто используемые слова добавляйте в **«Всегда конвертировать»** (или **«Никогда не конвертировать»**), и Nabira будет обрабатывать их как вам нужно.
